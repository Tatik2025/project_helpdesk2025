...
class TaskDetailForm(forms.Form):
    ...
    task_id = forms.IntegerField(label="Заявка №",required=False, widget=forms.TextInput(
    ...
    completion_Date_plan = forms.DateTimeField(label="Дата выполнения (план)", required=False, widget=forms.DateTimeInput(
        attrs={'type': 'datetime-local', 'class': 'form-control datetimepicker-input'}))

    completion_Date_actual = forms.DateTimeField(label="Дата выполнения (факт)",required=False, widget=forms.DateTimeInput(
        attrs={'class': 'form-control datetimepicker-input','disabled': 'disabled'}))

    Date_of_Adoption = forms.DateTimeField(label="Дата принятия в работу",required=False, widget=forms.DateTimeInput(
        attrs={'type': 'datetime-local', 'class': 'form-control datetimepicker-input', 'disabled': 'disabled'}))

    created_at = forms.DateTimeField(label="Дата создания", required=False, widget=forms.DateTimeInput(
        attrs={'class': 'form-control datetimepicker-input', 'disabled': 'disabled'}))

    author_id = forms.ModelChoiceField(queryset=class_user.objects,required=False, label="Автор",
                                       widget=forms.Select(attrs={'class': 'form-control', 'disabled': 'disabled'}))

    class Meta:
        fields=(
            'task_id','type_task_id','theme','priority_id','status_id','description','department_id','user_id','author_id',
            'created_at','completion_Date_actual','completion_Date_plan','Date_of_Adoption',)
...
class TaskCreateForm(forms.ModelForm):
    ...
    user_id = forms.ModelChoiceField(queryset=class_user.objects,required=False, label="Исполнитель",
                                     widget=forms.Select(attrs={'class': 'form-control'}))
